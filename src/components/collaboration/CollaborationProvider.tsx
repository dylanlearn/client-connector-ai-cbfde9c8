...
export const CollaborationProvider: React.FC<CollaborationProviderProps> = ({ 
  ...
  const defaultUserId = useUser();

  // If a documentId is provided as a prop, use it to initialize the context
  React.useEffect(() => {
    if (initialDocId) {
  ...
  const updateUserPresence = (presence: Partial<any>) => {
    ...
    // Update the user's presence in the document
    if (initialUserId || defaultUserId) {
      dispatch({
        type: 'UPDATE_USER_PRESENCE',
        payload: {
          userId: initialUserId || defaultUserId,
          presence: {
            ...presence,
